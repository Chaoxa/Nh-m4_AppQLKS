...
{
  global $error;
  if (!empty($error[$label_field])) return "<p class ='error text-danger'> $error[$label_field] </p>";
}
...
{
  global $notify;
  if (!empty($notify[$label_field])) return "<p class ='notify text-success pt-4'> $notify[$label_field] </p>";
}
